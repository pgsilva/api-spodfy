...
class SpotifyResource(val spotifyService: SpotifyService) : BaseResource() {
    ...
    @GetMapping("/aws/redirect_login")
    fun redirectAWSLoginSpotify(): ResponseEntity<Any>? {
        val scopes: String =
            "user-read-private " +
                    "user-read-email " +
                    "user-read-playback-state " +
        ...
        projectUrl += "&client_id=$SPOTIFY_CLIENT_ID"
        projectUrl += "&scope=${URLEncoder.encode(scopes, StandardCharsets.UTF_8.toString())}"
        projectUrl += "&redirect_uri=${URLEncoder.encode(SPOTIFY_REDIRECT_URI_AWS, StandardCharsets.UTF_8.toString())}"
    ...
    fun callbackLoginSpotify(
        ...
        return try {
            ...
            ResponseEntity.ok(spotifyService.atualizaPermissaoUsuarioLogado(code, getNrIpAdress(), false))
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    @GetMapping("/aws/callback")
    fun callbackAWSLoginSpotify(
        @RequestParam code: String?,
        @RequestParam(required = false) error: String?,
        @RequestParam(required = false) state: String?
    ): ResponseEntity<Usuario>? {
        return try {
            ...
            ResponseEntity.ok(spotifyService.atualizaPermissaoUsuarioLogado(code, getNrIpAdress(), true))
        } catch (e: Exception) {
            e.printStackTrace()
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build()
        }
